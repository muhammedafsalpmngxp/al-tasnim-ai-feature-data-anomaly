...
task_code                       e.g. 'FLME1180-30356'
 └ text before the FIRST '-'  = activity_id           'FLME1180'
    → dbo.mapping_master.Activity_ID          (CAST to nvarchar — it is a `text` column)
       → .Old_Activity_Code                            'FL-ME-ML08-03'
...
- ⚠ `mapping_master.Activity_ID` is a legacy `text` column. Comparing it directly FAILS with
  "the data types text and nvarchar are incompatible". ALWAYS join as
  `CAST(m.Activity_ID AS nvarchar(50)) = <activity_id>`.
- ⚠ USE `Old_Activity_Code`, NEVER `New_Activity_Code`. `activity_master_csv` is keyed on the OLD
  scheme: 246 of 247 Old codes match it, and ZERO New codes do. Joining on `New_Activity_Code`
  returns a NULL WBS for every row.
- `task_code` is an INTERNAL key. Use it ONLY to derive `activity_id`; NEVER display it when
  the question asks about a well's ACTIVITIES — identify an activity by its activity code
  (`Old_Activity_Code`), and list them DISTINCT.
...
SELECT DISTINCT a.activity_id, m.Old_Activity_Code AS activity_code
...
SELECT a.task_code, a.activity_id, m.Old_Activity_Code AS activity_code,
...
In every shape the second hop is
`LEFT JOIN dbo.activity_master_csv amc ON amc.activity_code = m.Old_Activity_Code`.
