...
def suggest_system_prompt(existing_check_titles: list[str]) -> str:
    return (
        ...
        "DO NOT propose a check for an EARLY/negative schedule variance (an actual date "
        "before its expected/target date). Confirmed live: a prior session proposed "
        "exactly this ('rig-on date earlier than expected') with severity_guess=high, "
        "even while its own hypothesis correctly called it an acceleration -- that is "
        "guard rule 1 above, violated in the metadata even when the prose got it right. "
        "If your hypothesis text describes something as early, ahead of schedule, or an "
        "acceleration, DO NOT call propose_check for it at all, regardless of what "
        "severity you would assign -- it is not a candidate check under any severity.\n\n"
